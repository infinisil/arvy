...
data ArvySpec a i r = forall msg s r' . ( Show (msg Node), HasState a s) => ArvySpec
  { arvyBehavior :: ArvyBehavior i msg r'
  -- ^ How the algorithm should behave for certain events occuring.
  , arvyRunner :: forall x . i ~ Node => Node -> a -> Sem r' x -> Sem (State s ': r) x
...
class HasState a s | a -> s where
  getState :: a -> s
  default getState :: s ~ () => a -> s
  getState = const ()
...
newtype GeneralArvy a r = GeneralArvy (forall i . NodeIndex i => ArvySpec a i r)
