...
Because the tree gets one more node in every iteration, the sequence for the number of edges between tree and non-tree nodes is $1\cdot n,\;2\cdot(n-1),\;3\cdot(n-2),\;\dots,\;(n-1)\cdot2,\;n\cdot1$. Every iteration therefore takes $\mathcal{O}(n^2)$ time to find the edge minimizing additional pair distance and another $\mathcal{O}(n)$ to update the data structure. The total complexity of this algorithm is therefore $\mathcal{O}(n\cdot(n^2+n))=\mathcal{O}(n^3)$. See \autoref{tree-examples} for an example of a tree constructed with this algorithm.
...
\addlegendentry {\hyperref[alg:lpm]{Local Pair Dist. Min.}}
\addplot table [y={inbetween-3-4-random}] {\data};
\addlegendentry {\hyperref[alg:frh]{Hop-based fixed ratio} with $t=\frac{3}{4}$}
